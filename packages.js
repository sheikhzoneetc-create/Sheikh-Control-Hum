import { listenPackagesData, listenFamilyPacks, createFamilyPack, assignCustomerToSlot, renewCustomer, deleteCustomer, getRemainingDays } from "./packages.js";
    ...
    return `
      <div class="card">
        ...
        <div style="font-size: 13px; color: var(--text-muted); margin: 6px 0; line-height: 1.5;">
          📦 <b>প্যাকেজ:</b> ${item.assignedPacks?.[0]?.packName || 'কাস্টম প্যাক'} | 🌐 <b>ডাটা:</b> ${item.assignedPacks?.[0]?.gb || '০'} GB | 📞 <b>মিনিট:</b> ${item.assignedPacks?.[0]?.min || '০'} Min<br>
          📅 <b>মেয়াদ:</b> ${item.joinDate} থেকে ${item.expiryDate} (🔄 রিনিউ: ${item.renewCount || 0} বার)
          ${item.hasYoutube ? `<br>🔴 <b>YouTube Premium:</b> <span style="color: var(--primary);">${item.youtubeGmail}</span>` : ''}
        </div>
...
window.handleDeleteCustomer = async function(id) {
  if (confirm("আপনি কি নিশ্চিত এই কাস্টমারটিকে মুছে ফেলতে চান?")) {
...
// Popups Form Handling (All Fields Included)
window.openModal = function(type) {
  if (type === 'pkgModal') {
    const name = prompt("কাস্টমারের নাম:");
    if (!name) return;
    const phone = prompt("কাস্টমারের ফোন নাম্বার:");
    const gb = prompt("কত জিবি দিতে চান? (যেমন: 20):") || "0";
    const min = prompt("কত মিনিট দিতে চান? (যেমন: 500):") || "0";
    const packName = prompt("প্যাকেজের নাম বা স্লট (যেমন: প্যাক #১ স্লট ২):") || "মেইন প্যাক";
    const hasYoutube = confirm("কাস্টমার কি YouTube Premium প্যাক নিয়েছে?");
    let youtubeGmail = "";
    if (hasYoutube) {
      youtubeGmail = prompt("কাস্টমারের YouTube প্রিমিয়াম জিমেইল:") || "";
    }

    assignCustomerToSlot({
      name,
      phone,
